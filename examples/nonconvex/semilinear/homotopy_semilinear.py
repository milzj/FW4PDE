...
from fw4pde.stepsize import QuasiArmijoGoldstein


def semilinear(n, u_init):
    ...
    plt.colorbar(c)
    plt.savefig("homotopy_solution_final_n_{}.pdf".format(n))
    plt.close()

    solution_best = sol["control_best"].data
    c = plot(solution_best)
    plt.colorbar(c)
    plt.savefig("homotopy_solution_best_n_{}.pdf".format(n))
    plt.close()
    ...
    np.savetxt("gradient_vec_n_{}.out".format(n), gradient_vec)
    np.savetxt("solution_vec_n_{}.out".format(n), solution_final.data.vector()[:])

    return solution_final.data
