...
def test_numpy_box_lmo(n, seed, beta, alpha):
    """
    ...
    numpy_box_lmo = NumpyBoxLMO(lb, ub, beta, alpha)
    ...
    if alpha > 0.0:
        _solution = projection(np.sign(-gradient) * np.maximum(np.abs(gradient) - beta, 0.0) / alpha, lb, ub)
    else:
        u = solution - gradient
